...
export default function DashboardAppPage() {
  ...
  useEffect(() => {
    const thongKeSoLuong = async () => {
      try {
        const url = `${process.env.REACT_APP_API_URL}/dashboard/soLuong`;
    ...
    const thongKeTreEm = async () => {
      try {
        const url = `${process.env.REACT_APP_API_URL}/dashboard/treem`;
        const { data } = await axios.get(url, { withCredentials: true });
        setTreEm(data);
      } catch (err) {
        console.log(err);
    ...
    <>
      ...
      <Container maxWidth="xl">
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} sm={6} md={3}>
            <AppWidgetSummary title="Nhà tài trợ" total={total.donViBaoTro} color="info" icon= {'fa6-solid:people-roof'} />
          </Grid>
